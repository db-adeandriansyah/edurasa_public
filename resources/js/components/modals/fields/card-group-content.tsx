...
import { contentField, ModalCustomInterface } from "../type";
    ...
    return (
        <Card className="flex flex-col gap-0 pt-0 pb-0 overflow-hidden rounded-t-2xl min-h-64  dark:bg-gray-400">
            ...
            <CardContent>
                {
                    dataField?.filter(s=>fieldGroupKey.includes(s.key)).map((field, key)=> 
                        <ResolveFieldByTypeOnly contentTabHeadersOnly={contentTabHeadersOnly} key={key} type={field} currentData={currentData} setCurrentData={setCurrentData}/>
                    )   
                }
            </CardContent>
        </Card>
